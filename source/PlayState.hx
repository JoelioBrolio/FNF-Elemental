...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'reunion' | 'myst':
				defaultCamZoom = 0.8;
				curStage = 'museum';
				var bg:BGSprite = new BGSprite('museumPillars', -400, -60, 0.8, 0.8);
				add(bg);

				var museumGrassyGrass:BGSprite = new BGSprite('museumGrassyGrass', -400, -60, 0.8, 0.8);
				add(museumGrassyGrass);

				var museumCoolBox:BGSprite = new BGSprite('museumCoolBox', -400, -60, 0.8, 0.8);
				add(museumCoolBox);

				var museumSidewalk:BGSprite = new BGSprite('museumSidewalk', -400, -60, 0.8, 0.8);
				add(museumSidewalk);

				var museumCameos:BGSprite = new BGSprite('museumCameos', -400, -60, 0.8, 0.8);
				add(museumCameos);
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			{
				if (SONG.song.toLowerCase() == "myst")
				{
					FlxG.sound.music.stop();
					vocals.stop();

					var endSongText = CoolUtil.coolTextFile(Paths.txt("myst/mystEndDialogue"));

					var endSongBox:DialogueBox = new DialogueBox(false, endSongText, true);

					schoolIntro(endSongBox);
					endSongBox.cameras = [camHUD];
					endSongBox.scrollFactor.set();
					endSongBox.finishThing = function()
					{
					...
					endSongBox.nextDialogueThing = startNextDialogue;
				}
				else
				{
					FlxG.sound.playMusic(Paths.music('freakyMenu'));
